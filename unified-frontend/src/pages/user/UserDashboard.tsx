      ...
      try {
        ...
        if (requestsResponse.ok) {
          ...
          if (requestsData.success) {
            // Filter requests so the dashboard only shows requests for the signed-in user.
            // Prefer matching by user id if available on the request (request.user._id or request.user.id),
            // otherwise fall back to matching by email.
            const allRequests: any[] = requestsData.data.requests || [];
            const currentUserId = (authUser as any)?.id || (authUser as any)?._id || null;
            const currentUserEmail = (authUser as any)?.email || null;

            const userRequests = allRequests.filter(r => {
              if (r.user && (r.user._id || r.user.id)) {
                return currentUserId && ((r.user._id === currentUserId) || (r.user.id === currentUserId));
              }
              if (r.email && currentUserEmail) {
                return String(r.email).toLowerCase() === String(currentUserEmail).toLowerCase();
              }
              return false;
            });

            const transformedRequests: Request[] = userRequests.map((req: any) => ({
              id: req._id,
              type: req.type,
