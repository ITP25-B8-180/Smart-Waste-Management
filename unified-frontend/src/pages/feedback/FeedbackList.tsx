...
export default function FeedbackList() {
    ...
    const loadFeedbacks = async () => {
        try {
            ...
            let feedbacks: Feedback[] = response.data.data.feedbacks || [];

            // If a normal user is viewing, only show their own feedbacks.
            // Admins (feedback_admin) should still see everything.
            const role = (authUser as any)?.role;
            const currentEmail = (authUser as any)?.email;
            if (authUser && role !== 'feedback_admin' && role !== 'admin' && currentEmail) {
                feedbacks = feedbacks.filter(f => String(f.email).toLowerCase() === String(currentEmail).toLowerCase());
            }
