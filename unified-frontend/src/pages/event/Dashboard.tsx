...
export default function EventDashboard() {
        ...
        try {
            ...
            try {
                ...
                realBookings = apiBookings.map((b: any) => {
                    // Ensure event is an object to avoid crashes when rendering
                    const ev = (b.event && typeof b.event === 'object') ? b.event : { title: String(b.event || ''), location: '' };
                    return {
                        _id: b._id,
                        event: ev,
    ...
    const formatSafe = (dateInput?: string | number | Date, fmt = 'MMM dd, yyyy') => {
        if (!dateInput) return '';
                                        ...
                                        <div className="flex justify-between items-start">
                                            <div className="flex-1">
                                                ...
                                                <p className="text-sm text-gray-500">
                                                    {formatSafe(event.date)}{event.time ? ` at ${event.time}` : ''}
                                                </p>
                            ...
                            <div className="space-y-4">
                                {bookings.slice(0, 5).map((booking) => (
                                    <div key={booking._id} className="border rounded-lg p-4">
                                        ...
                                        <p className="text-sm text-gray-500">
                                            Booked on {formatSafe(booking.bookingDate)}
                                        </p>
